...
class ProblemError(BaseModel):
    ...
    location: StrictStr
    pointer: StrictStr
    __properties: ClassVar[List[str]] = ["code", "detail", "location", "pointer"]

    @field_validator('location')
    def location_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['body', 'header', 'path', 'query']):
            raise ValueError("must be one of enum values ('body', 'header', 'path', 'query')")
        return value
