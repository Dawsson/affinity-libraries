...
class GetWebhookEventResponseDeliveriesInner(BaseModel):
    """
    GetWebhookEventResponseDeliveriesInner
    """ # noqa: E501
    attempt_count: Union[StrictFloat, StrictInt] = Field(alias="attemptCount")
    endpoint_id: Annotated[str, Field(strict=True)] = Field(alias="endpointId")
    id: Annotated[str, Field(strict=True)]
    last_error: Optional[StrictStr] = Field(alias="lastError")
    status: StrictStr
    __properties: ClassVar[List[str]] = ["attemptCount", "endpointId", "id", "lastError", "status"]
    ...
    def id_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^whd_[0-9a-hjkmnp-tv-z]{26}$", value):
            raise ValueError(r"must validate the regular expression /^whd_[0-9a-hjkmnp-tv-z]{26}$/")
        return value

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['dead_letter', 'delivered', 'failed', 'pending']):
            raise ValueError("must be one of enum values ('dead_letter', 'delivered', 'failed', 'pending')")
        return value
